...
def create_user():
    ...
    if nombre_real_manual and nombre_real_manual.strip():
        ...
        last_quemado = User.query.filter(User.nombre_real.like(f"{nombre_real_quemado} %")).order_by(User.id.desc()).first()
        if last_quemado:
    ...
    new_user = User(username=username, nombre_real=nombre_real, mail=mail, password=password_hash, is_admin=is_admin)
